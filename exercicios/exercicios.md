...
```sh
Não funcionou: $ export RELEASE=$(curl -s https://api.github.com/repos/kubeless/kubeless/releases/latest | grep tag_name | cut -d '"' -f 4)
$ export RELEASE=$(curl -s https://api.github.com/repositories/73902337/releases/latest | grep tag_name | cut -d '"' -f 4)
